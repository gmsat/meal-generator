  ...
  const GenerateAgainButton = () => {
    ...
    return (
      <Grow in={hiddenGenerator}>
        <Button sx={{display: !hiddenGenerator && "none"}} color={"primary"} variant={"contained"}
                onClick={handleClick}>Generate new recipes</Button>
      </Grow>
    )
  }
        ...
        <Card variant={"outlined"} sx={{display: hiddenGenerator && "none", width: "100%", ":hover": {boxShadow: 3}}}>
          <Grid container flexDirection={"column"} sx={inputStyle} gap={2}>
            ...
            <MealParamsToggle value={diet} setValue={setDiet} size={"small"} orientation={"horizontal"}>
              <TooltipToggleButton title={"No meat, poultry, fish, dairy and eggs"}
                                   disableRipple
                                   sx={customToggleStyle}
                                   value={"vegetarian"}>
                Vegetarian
              </TooltipToggleButton>
              <TooltipToggleButton title={"Includes dairy products"}
                                   disableRipple
                                   sx={customToggleStyle}
                                   value={"lacto-vegetarian"}>
                Lacto-vegetarian
              </TooltipToggleButton>
              <TooltipToggleButton title={"Includes eggs, but not dairy products"}
                                   disableRipple
                                   sx={customToggleStyle}
